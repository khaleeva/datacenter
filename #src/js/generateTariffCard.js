...
function generateTariffDetails(baseSum, vps_addons, type) {
    const light_state = createStateObject(
        vps_addons,
        vps_card_values.light_state_values,
        "black",
    );
    const optima_state = createStateObject(
        vps_addons,
        vps_card_values.optima_state_values,
        "black",
    );
    const best_state = createStateObject(vps_addons, vps_card_values.best_state_values, "white");
    const max_state = createStateObject(vps_addons, vps_card_values.max_state_values, "white");
    ...
    switch (type) {
        case "light":
            return `<div class="tariff__card-detail">${generateBlockDetail("black", totalLightPrice, link_light, light_state.ram, light_state.space, light_state.ip4, light_state.cpu, light_state.panel, light_state.support)}</div>`;
        case "optima":
            return `<div class="tariff__card-detail">${generateBlockDetail("black", totalOptimaPrice, link_optima, optima_state.ram, optima_state.space, optima_state.ip4, optima_state.cpu, optima_state.panel, optima_state.support)}</div>`;
        case "best":
            return `<div class="tariff__card-detail">${generateBlockDetail("white", totalBestPrice, link_best, best_state.ram, best_state.space, best_state.ip4, best_state.cpu, best_state.panel, best_state.support)}</div>`;
        case "max":
